...
Laravel Artisan コマンドの実装とレビューを本番環境で実証済みのパターンと Laravel 9+ ベストプラクティスに従って支援する包括的なスキルです。

## 概要

このスキルは、JobAntenna v4 モノレポプロジェクトから抽出された実戦的なパターンと、Laravel 公式ドキュメントのベストプラクティスを統合し、高品質な Artisan コマンドの実装とレビューをサポートします。
...
- **コマンドレビュー**: 専門エージェントによる実装パターンの品質評価とベストプラクティス準拠チェック
...
### コマンドレビュー
- 既存のコマンド実装が実証済みパターンに準拠しているかチェックする
- ベストプラクティスに基づいた改善提案を受ける
- コードの品質とメンテナンス性を評価する
...
├── agents/
│   └── laravel-command-reviewer.md  # コマンドレビュー専門エージェント
└── scripts/                     # ヘルパースクリプト
```
